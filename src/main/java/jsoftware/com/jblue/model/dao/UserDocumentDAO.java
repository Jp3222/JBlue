...
public class UserDocumentDAO extends AbstractDAO {
     ...
     * de la lista.
     */
    public boolean insert(JDBConnection connection, List<UserDocumentDTO> doc_list) throws SQLException, CorruptInsertionException {
        boolean res = false;
        String query = """
                            INSERT INTO usr_user_document
                            (user, document_name, document_path, doc_file) 
                            VALUES
                            (?,?,?,?)
                       """;
