...
public class VUsuariosR extends VistaSimple {
    ...
    private void setInfoEnPantalla(OUsuarios o) {
        ...
        if (!o.isTitular()) {
            var lista = cache.stream().filter((t) -> t.getTitutlar().equalsIgnoreCase(o.getTitutlar())).toList();
            campo_titular.setSelectedItem(lista.get(0).getStringR());
        }
        var calle = EnvUsuario.getCalleEnCache(o.getCalle());
        campo_calle.setSelectedItem(calle.getStringR());
        var tipotoma = EnvUsuario.getTipoDeTomaEnCache(o.getToma());
        campo_tipo_toma.setSelectedItem(tipotoma.getStringR());
