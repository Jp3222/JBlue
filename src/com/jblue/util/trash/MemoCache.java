...
public class MemoCache<T extends Objeto> {
    ...
    private void getIdsMinMax() {
        try {
            ...
            try (ResultSet _count = cn.queryResult("SELECT count(id) FROM " + operaciones.getTABLA())) {
                if (_count.next()) {
                    count = _count.getInt(1);
                }
            }
    ...
    public void cargar() {
        ...
        ArrayList<T> aux;
        if (q.isEmpty()) {
            aux = operaciones.getLista(query);
        } else {
    ...
    private void newCargar() {
        ...
        ArrayList<T> aux;
        if (q.isEmpty()) {
            aux = (ArrayList<T>) operaciones.select(null, null).get();
        } else {
            aux = (ArrayList<T>) operaciones.select(null, q.toString()).get();
        }
